...
function makePlayerState(
  overrides: Partial<PlayerUnitState> = {},
): PlayerUnitState {
  return {
    level: 1,
    isInCamp: false,
...
describe("player persistence — revive", () => {
  it("revived player persists as alive with restored HP", () => {
    ...
    state = {
      ...state,
      units: killedUnits,
      occupancy: buildOccupancy(killedUnits, state.deployments),
    };
    ...
    expect(after.hero!.currentHp).toBe(
      computeReviveHp({ maxHp: 100 }, { level: 1 }),
    );
  });
