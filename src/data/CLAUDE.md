...
- `skillDefinitions.ts` — 28 combat skills, multiplier/effect matrices, level tables; exports `SKILLS`, `MULTIPLIER_MATRICES`, `EFFECT_MATRICES`, `INSTANT_EFFECT_MATRICES`, `LEVELED_EFFECTS`, `DAMAGE_MODIFIER_LEVELS`, `VAMPIRISM_LEVELS`; runtime helpers live in `src/battle/skillDefinitionRuntime.ts`. `MULTIPLIER_MATRICES` are used by `damage`, `heal`, and `apply_periodic_hp_effect`. `EFFECT_MATRICES` are shape-only registries for `apply_stat_effect` (cell multipliers are not used for magnitude).
