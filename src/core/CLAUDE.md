...
- **Debug session lifecycle has exactly one owner: `debugLifecycle.ts`.** `phaseActionEffects`
  ...
  teardown does not cover. Dispatched *from* battle both clears run — the explicit one and the
...
- **Battle exit ordering is load-bearing.** `exit_battle` runs roster persistence
  (`applyBattleExitRosterEffect`) → campaign world consequence (`applyBattleWorldConsequence`) →
  generic teardown, in that order: both effects need the runtime still installed, so disposal
  cannot move ahead of either. Each of the two resolves and validates the runtime inside its own
  body rather than trusting the other to have done it
- **Generic battle-runtime teardown is structural, not an action catalogue.**
  ...
  the previous phase is `battle` and the resolved phase is not — comparing phase *types*, never
...
**Acceptable `GameState` reads:** core state initialization, the effects domain owners (`campaignLifecycle`, `debugLifecycle`, `campaignWorldTransitions`' callers, `battlePhaseEffects`, `phaseHandlers/*`), `rebuildPhaseSnapshot()`, `derivePhaseTransitionMetadata()`, `requireBattleRuntimeForPhase()`. Note `phaseActionEffects` itself is **not** on this list: the facade sequences owners and never touches a store.
