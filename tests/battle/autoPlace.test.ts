...
function playerCandidate(templateId: string): PlayerPlacementCandidate {
  return {
    templateId,
    shape:       { offsets: [{ dr: 0, dc: 0 }] },
    rowTrait:    'front',
    savedAnchor: null,
    createUnit:  (id) => makeUnit({ id, templateId, side: 'player' }),
  };
}
...
describe('autoPlacePlayer', () => {
  ...
  it('places candidates on the field until capacity, then benches overflow', () => {
    ...
    const result     = autoPlacePlayer(emptyState(), candidates, 3);

    const deployments = [...result.deployments.values()];
    expect(deployments.filter(d => d.kind === 'field')).toHaveLength(6);
    expect(deployments.filter(d => d.kind === 'bench')).toHaveLength(1);

    const benchUnitId = [...result.deployments.entries()]
      .find(([, d]) => d.kind === 'bench')![0];
    expect(result.occupancy.unitToCells.has(benchUnitId)).toBe(false);
    for (const [id, dep] of result.deployments) {
      if (dep.kind === 'field') expect(result.occupancy.unitToCells.has(id)).toBe(true);
    }
  });
  ...
  it('creates each overflow runtime unit exactly once', () => {
    ...
    const overflowUnits = [...result.units.values()].filter(u => u.templateId === overflowTpl);
    expect(overflowUnits).toHaveLength(1);

    const dep = result.deployments.get(overflowUnits[0].id);
