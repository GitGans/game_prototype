...
- **Reverse coverage closes the level below the facades (Stage 4C).** Stage 4A guarantees a facade's imports match its registration; it says nothing about what those registered collaborators may import, which is how a hidden monolith regrows one level down — `battlePhaseEffects` acquiring snapshot construction, `campaignLifecycle` absorbing debug lifecycle, `worldMapProjection` importing `GameState` and becoming a mutation route. So the required set is **derived** from the real `ORCHESTRATION_COLLABORATOR_REGISTRY` (every non-neutral edge, deduplicated across facades) by `scripts/orchestration-collaborator-coverage.mjs`, never hand-listed, and every derived module must carry **exactly one** canonical exact import policy — from `PHASE_HANDLER_IMPORT_POLICIES`, `RUNTIME_OWNERSHIP_IMPORT_POLICIES` or `ORCHESTRATION_COLLABORATOR_IMPORT_POLICIES`. Zero policies and two policies are both violations: with two, neither is authoritative.
- Covered policies and actual imports match **in both directions**, module by module, so a stale allowed import is rejected as the dormant permission it is. A Stage 4C *registry entry* with no corresponding collaborator is rejected too — that rule is one-sided by design, since the other two registries legitimately govern non-collaborators (`phaseHandlers/battlePhaseHandler`, the runtime storage cell, the write gateway), each already covered by its own rule. `neutral-contract` edges are the only exemption; there is **no** direct-collaborator exclusion mechanism, and an unknown role fails closed rather than becoming implicitly exempt. Compilation is fail-closed throughout: missing, duplicate, ambiguous, malformed or stale configuration yields **no** enforceable coverage rather than a partially active policy, and a collaborator that resolves to no source file is a diagnosed error, never a module read as having zero imports.
- Coverage stops after one collaborator level. The transitive graph is deliberately not registered, so ordinary domain work does not become boundary-registry maintenance.
- **The limit of all of this, stated plainly.** These are dependency policies. They constrain which modules a module may reach; they do **not** inspect a function body, and they do **not** distinguish which exported method of an allowed module a caller uses. `phaseTransitionMetadata` and `phaseSnapshotRebuilder` import `GameState` and `PlayerSessionStore` directly, and both of those expose replacement methods — so `authoritative-state-reader` is a reviewed classification, not a mechanically enforced read-only capability, and no persistent-state readonly guarantee exists. Separately, and unchanged since Stage 4A: the suites re-derive real-source parity independently, but nothing asserts that `check-boundaries.mjs` is *wired* correctly — a subprocess test of the executable is out of scope. Registry correctness is tested; checker wiring is reviewed.
