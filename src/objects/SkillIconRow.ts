...
export class SkillIconRow extends Phaser.GameObjects.Container {
  ...
  render(skills: SkillIconSnapshot[]): void {
    ...
    skills.forEach((skill, i) => {
      ...
      const bg = this.scene.add.rectangle(0, cy, CELL_SIZE, CELL_SIZE, 0x2a2a3a).setOrigin(0, 0);
      ...
      const icon: Phaser.GameObjects.GameObject = this.scene.textures.exists(textureKey)
        ...
        : this.scene.add.rectangle(0, cy, CELL_SIZE, CELL_SIZE, 0x666666).setOrigin(0, 0);

      const border = this.scene.add.rectangle(0, cy, CELL_SIZE, CELL_SIZE, 0, 0)
        .setOrigin(0, 0).setStrokeStyle(1, 0x556677);

      const label = this.scene.add.text(
        CELL_SIZE + NAME_OFFSET_X,
        cy + CELL_SIZE / 2,
        skill.name,
        { fontSize: `${Math.round(13 * LAYOUT_SCALE)}px`, color: '#cccccc' },
      ).setOrigin(0, 0.5);
